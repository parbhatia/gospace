...
const ProduceControlButton = ({
   ...
   <button
      onClick={onClick}
      type="button"
      className={`flex items-center w-full justify-center h-full p-2 m-0.5 text-base font-semibold text-center transition fill-current bg-white duration-200 border-2 border-gray-900 rounded-lg shadow-md ease-i hover:bg-red-500 hover:text-white${selected ? "border-0  bg-red-500text-white " : ""
         }`}
   >
      {children}
   </button>
